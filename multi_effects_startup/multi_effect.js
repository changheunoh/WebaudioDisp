	...
	var biquad_bypass = false;
			...
			//variables for bypass filter //choh -----
			var biquadon = context.createGain();
			var biquadoff = context.createGain();
			biquadon.gain.value = 1;
			biquadoff.gain.value = 0;
			var biquad_to_delay_buffer = context.createGain();
			/////////////////////////////////////////// //----choh
	...
	var delay_bypass = false;
		...
		//variables for bypass delay //choh -----
			var delayon = context.createGain();
			var delayoff = context.createGain();
			delayon.gain.value = 1;
			delayoff.gain.value = 0;
			var delay_to_reverb_buffer = context.createGain();
	/////////////////////////////////////////// //----choh
	...
	var impulseResponse = null;
		...
		//variables for bypass filter //choh -----
		var reverbon = context.createGain();
		var reverboff = context.createGain();
		reverbon.gain.value = 1;
		reverboff.gain.value = 0;
		/////////////////////////////////////////// //----choh
	...
	function playSound(anybuffer) {
		...
		//choh-----
		//filter parts
			//connect to 2 gain switches
			source.connect(biquad);
			source.connect(biquadoff);

			biquad.connect(biquadon);
			biquadon.connect(biquad_to_delay_buffer);
			biquadoff.connect(biquad_to_delay_buffer);

	//delay parts
		biquad_to_delay_buffer.connect(delay);
		biquad_to_delay_buffer.connect(delayoff);

		delay.connect(delayon);
		delay.connect(feedbackGain);
		feedbackGain.connect(delay);
		delayon.connect(delay_to_reverb_buffer);
		delayoff.connect(delay_to_reverb_buffer);

	//reverb part
		delay_to_reverb_buffer.connect(convolver);
		delay_to_reverb_buffer.connect(reverboff);

		convolver.connect(wetGain);
		wetGain.connect(reverbon);
		delay.connect(dryGain);
		dryGain.connect(reverbon);
		reverbon.connect(context.destination);
		reverboff.connect(context.destination);
	/////////////////////////////////////////// //----choh
...
//choh ---- /////////////////////////////////////////////////////
	function toggleFilterBypass() {
			if ( biquad_bypass ) {
				...
				biquad_bypass = true;
				biquadon.gain.value = 0;
				biquadoff.gain.value = 1;
			}
		}
		...
		function toggleDelayBypass() {
			if ( delay_bypass ) {
				...
				delay_bypass = true;
				delayon.gain.value = 0;
				delayoff.gain.value = 1;
			}
		}
		...
		function toggleReverbBypass() {
			if ( reverb_bypass ) {
				...
				reverb_bypass = true;
				reverbon.gain.value = 0;
				reverboff.gain.value = 1;
			}
		}
		///////////////////////////////////////////////////// //---choh
